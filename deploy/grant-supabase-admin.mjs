export async function grantSupabaseAdmin({ email = process.env.TEAM_MEMBER_EMAIL, env = process.env, fetchImpl = fetch } = {}) {
  ...
  const lookup = await fetchImpl(`${supabaseUrl}/rest/v1/users?email=eq.${encodeURIComponent(normalizedEmail)}&select=openId,email,role&limit=2`, {
  ...
  if (!Array.isArray(records) || records.length !== 1 || !records[0]?.openId) throw new Error(`Nenhum perfil único do ERP foi encontrado para ${normalizedEmail}. A pessoa deve entrar uma vez antes de ser aprovada.`);
