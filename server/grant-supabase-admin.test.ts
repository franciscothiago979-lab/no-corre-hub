...
describe("aprovação direta de integrante", () => {
  it("localiza a conta pelo e-mail e a promove a administradora", async () => {
    const fetchImpl = vi.fn()
      .mockResolvedValueOnce(new Response(JSON.stringify([{ openId: "supabase:member", email: "membro@example.com", role: "user" }, { openId: "supabase:other", email: "other@example.com", role: "user" }]), { status: 200 }))
